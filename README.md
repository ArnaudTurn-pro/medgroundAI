...
**Grounded Graph-RAG over the medical research literature — no clinical claim ships without a real, retrievable source.**

medground pairs a capable LLM (Claude Opus) with a finite, citable corpus of biomedical research
and a **deterministic grounding gate**. Retrieval is hybrid — dense vectors + BM25 + a MeSH
knowledge graph — and every answer must pass a provenance check before it reaches you: each claim
has to cite a `paper_id` that the system can actually find. A confident, unsupported sentence is
treated as a bug, not a stylistic choice.
...
Large language models hallucinate — and for clinical questions a fluent, wrong, *uncited* answer
is dangerous. medground's thesis is that an LLM is only trustworthy on clinical questions when
it is **forced to retrieve, forced to cite, and mechanically checked**. The system provides the
...
| `MG_NCBI_EMAIL` | — | Sent to NCBI as a courtesy (not a secret). |
| `MG_NCBI_API_KEY` | — | Raises NCBI rate limit 3 → 10 req/s. |
| `MG_CIVIC_API_KEY` | — | Optional CIViC bearer token (reads are open; not required). |
...
| `MG_ENV_FILE` | — | Explicit path to a `.env` to load first. |
...
- **CIViC** (GraphQL) — curated, expert-moderated **variant → disease → therapy** evidence. Each item
  ...
  A–E evidence levels. PubMed `citationId`s link CIViC items back to the literature.
